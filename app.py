...
# =========================================================
# CSS / UI (LOGIN ESCRIBIBLE + SIN FRANJA BLANCA)
# =========================================================
def inject_css(is_login: bool):
    ...
    <style>
      ...
      /* ===== LOGIN (NO fixed para que Streamlit no buguee) ===== */
      .login-overlay {{
        position: fixed;
        inset: 0;
      ...
      /* ===== QUITAR “FRANJA BLANCA” (widgets flotantes) ===== */
...
def get_gsheet_client() -> gspread.Client:
    ...
    if "gcp_service_account" in st.secrets:
        ...
        if not gac:
            raise RuntimeError("Falta gcp_service_account en Secrets (Streamlit Cloud) o GOOGLE_APPLICATION_CREDENTIALS (local).")
...
def ensure_admin_seed_and_optional_reset():
    ...
    - Si en Secrets pones ADMIN_RESET_PASSWORD: fuerza password admin.
    """
    _, wss = get_db()
...
def normalize_config(raw: Any) -> Dict[str, Any]:
    ...
    # dict con otra clave / o dict de equipos
...
def approve_submission(submission_id: str, supervisor: Dict[str, Any], conforme: str, observaciones: str, firma_supervisor_b64: str):
    ...
    ok = ws_update_row_by_key(
        wss["submissions"], "submission_id", submission_id,
        {"status": "APROBADO", "updated_at": _now_iso()}
    )
    if not ok:
        raise RuntimeError("No pude actualizar status del submission.")

    sub, df_items, _df_photos, _appr = get_submission_detail(submission_id)
    if not sub:
...
if role == "operador":
    if not equipos_list:
        ...
        if isinstance(RAW_CONFIG, dict):
            st.info(f"Claves encontradas en tu JSON: {list(RAW_CONFIG.keys())[:40]}")
        st.stop()
    ...
    if page == "Llenar checklist":
        ...
        st.markdown(
            f'<span class="pill">Equipo: {equipo}</span><span class="pill">Ítems: {len(items)}</span>',
            unsafe_allow_html=True
        )
        st.markdown('<div class="hr"></div>', unsafe_allow_html=True)
        ...
        st.dataframe(
            dfo[["submission_id", "date", "equipo", "estado_general", "status", "updated_at"]],
        ...
        if st.button("Buscar PDF"):
            ...
            if not sub:
                ...
                st.download_button(
                    "Descargar PDF",
                    data=b64_to_bytes(appr["pdf_b64"]),
...
# =========================================================
    ...
    if page == "Usuarios":
        ...
        st.dataframe(
            dfp[["submission_id", "created_at", "date", "equipo", "operador_full_name", "estado_general", "status"]],
            use_container_width=True, hide_index=True
        )
        ...
        if sid:
            ...
            st.markdown("##### Ítems")
            st.dataframe(
                df_items[["item_id", "item_text", "estado", "comentario"]] if not df_items.empty else pd.DataFrame(),
                use_container_width=True, hide_index=True
            )
        ...
        st.markdown('<div class="hr"></div>', unsafe_allow_html=True)
        st.dataframe(
            dff[["submission_id", "date", "equipo", "operador_full_name", "estado_general", "status", "updated_at"]].head(400),
            use_container_width=True, hide_index=True
        )
